...
test('real HTTP API and browser CSP upload, render, play, and ripple-edit a generated movie',{skip:!python||!hasBrowser?'Requires media Python and optional tests/ui Playwright installation':false,timeout:120000},async()=>{
 ...
 try{
  ...
  page.on('dialog',dialog=>/^(录一个新的完整版本|确认把这段停顿剪短吗)/.test(dialog.message())?dialog.accept():dialog.dismiss())
  ...
  // A controlled Agent callback exercises the actual review→notice→render loop;
  // this is not a live-model inference test.
  await core.bindSession('fixture-studio',p.id)
  core.setAgentStatusReader(async()=>({liveStatus:'idle',lastTurnReason:'completed',messages:[{id:'visible-reply',seq:1,text:'这段空白可以更紧凑，请先试听，再确认。',interrupted:false}]}))
  let revised
  core.onAgentNotice(async notice=>{if(notice.kind==='review'&&notice.status==='approved'){const latest=await core.dispatch('project.get',{}, {projectId:p.id,sessionId:'fixture-studio'});revised=await core.dispatch('movie.render',{expectedRevision:latest.revision},{projectId:p.id,sessionId:'fixture-studio'})}return {delivery:'queued'}})
  const proposed=await core.dispatch('timeline.propose',{expectedRevision:p.revision,operation:op},{projectId:p.id,sessionId:'fixture-studio'})
  assert.ok(proposed.requiresConfirmation)
  const card=page.locator(`[data-proposal-id="${proposed.id}"]`)
  await card.waitFor();await page.locator('#agent-messages').getByText('这段空白可以更紧凑，请先试听，再确认。').waitFor()
  await page.locator('#recording-player').evaluate(a=>a.muted=true)
  await card.getByRole('button',{name:'试听原段'}).click()
  await page.waitForFunction(()=>!document.getElementById('recording-player').paused)
  await card.getByRole('button',{name:'确认剪短'}).click()
  for(let i=0;i<200&&!revised;i++)await delay(25)
  assert.ok(revised,'Approved review must wake the originating Agent callback')
  const final=await wait(revised.id)
