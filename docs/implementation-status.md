...
v0.1.0 已经通过 PR #2 合并并发布预览版；发布状态仍以 GitHub PR/Release 为准，不把本地测试自动当成远端发布。

## 安装/配置/使用图文补充（Issue #3）

- 新增 `docs/quick-start.md`，README嵌入工作室导览并链接成人安装、配置、孩子创作和审片四段。
- `docs/media` 包含2张解释型安装/配置图、4张真实隔离工作室截图和1个GIF；不用生产认证、真实家庭素材或模型调用。
- GIF为1000×742、5关键帧、7.70秒、316901字节；完整帧尺寸/时长/字节/hash通过独立解析及Pillow核对，首/中/末帧做可读性检查。
- 复现脚本在 `docs/scripts`；原始捕获与临时数据在被忽略的 `.test-output`，临时HTTP/Core资源已清理。生成媒体采用MIT许可，素材来源和无真实推理边界见 `docs/media/README.md`。
- 相对链接、导航锚点和媒体清单通过 `tests/docs-media.test.mjs`；npm包仍只含文本/代码，不包含GitHub图文媒体，没有修改业务行为。
- 图文更新通过单独的 `feature/docs-visual-guide` 分支与PR交付，不因旧版本已合并而自动合并本次PR。
